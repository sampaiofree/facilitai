...
class ClienteScheduledMessageController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $timezone = $this->resolveTimezoneForCliente($cliente);

        $query = ScheduledMessage::query()
            ->with(['clienteLead.cliente', 'assistant', 'conexao'])
            ->whereHas('clienteLead', fn ($q) => $q->where('cliente_id', (int) $cliente->id))
        ...
        return view('cliente.mensagens-agendadas.index', [
    ...
    {
        $cliente = $request->user('client');
        abort_unless($this->canAccess($scheduledMessage, (int) $cliente->id), 403);

        $timezone = $this->resolveTimezoneForCliente($cliente);

        $scheduledMessage->loadMissing(['clienteLead.cliente', 'assistant', 'conexao']);
    ...
    private function resolveTimezoneForCliente(Cliente $cliente): string
    {
        ...
        return app(ScheduledMessageService::class)->resolveTimezoneForUser($cliente->user);
    }

    private function canAccess(ScheduledMessage $scheduledMessage, int $clienteId): bool
    {
        $scheduledMessage->loadMissing('clienteLead');

        return (int) ($scheduledMessage->clienteLead?->cliente_id ?? 0) === $clienteId;
    }
