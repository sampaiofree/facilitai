...
class ProcessSequences extends Command
{
        ...
        SequenceChat::with(['sequence', 'chat.tags', 'chat.instance'])
            ...
            ->where(function ($q) use ($nowUtc) {
                $q->whereNull('proximo_envio_em')
                  ->orWhere('proximo_envio_em', '<=', $nowUtc);
            })
            ->chunk(100, function ($batch) use ($nowLocal) {
                foreach ($batch as $inscricao) {
                    ...
                    if (!$this->prontoParaDisparar($inscricao, $step, $nowLocal)) {
                    ...
                    try {
                        ...
                        $this->avancarPasso($inscricao, $step, $nowLocal);
                    } catch (\Throwable $e) {
                        ...
                        $this->avancarPasso($inscricao, $step, $nowLocal, true);
                    }
                }
            });
    ...
    private function cancelarPorTags(SequenceChat $inscricao, $incluir, $excluir, $tagsChat): void
    {
        ...
        Log::info('Sequência cancelada por tags', [
    ...
    private function obterPassoAtual(SequenceChat $inscricao): ?SequenceStep
    {
        ...
        if ($step) {
            ...
            $inscricao->proximo_envio_em = $inicioLocal->clone()->setTimezone('UTC');
            $inscricao->save();
        }
