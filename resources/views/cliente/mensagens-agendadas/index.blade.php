        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Historico de agendamentos dos leads do seu cliente.</p>
                </div>
            ...
            <form method="GET" class="mt-4 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-5">
                ...
                <div class="md:col-span-5 flex items-center gap-3">
                    ...
                    <a href="{{ route('cliente.mensagens-agendadas.index') }}" class="text-xs font-semibold text-slate-500 hover:text-slate-900">
                        Limpar
                    </a>
                </div>
            </form>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($scheduledMessage->status === 'pending')
                                        ...
                                        <form method="POST" action="{{ route('cliente.mensagens-agendadas.cancel', $scheduledMessage) }}" onsubmit="return confirm('Cancelar este agendamento?');">
                                            @csrf
                                            @method('PATCH')
...
@push('scripts')
    <script>
        (() => {
            ...
            const showUrlTemplate = @json(route('cliente.mensagens-agendadas.show', ['scheduledMessage' => '__SCHEDULED_ID__']));
            const updateUrlTemplate = @json(route('cliente.mensagens-agendadas.update', ['scheduledMessage' => '__SCHEDULED_ID__']));
