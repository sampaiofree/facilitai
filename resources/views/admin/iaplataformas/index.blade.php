...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-semibold text-slate-900">IA Plataformas</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($iaplataformas as $tpl)
                    <tr class="hover:bg-slate-50">
                        <td class="px-5 py-4 font-medium text-slate-800">{{ $tpl->nome }}</td>
                        <td class="px-5 py-4">
                            @if($tpl->ativo)
                        ...
                        <td class="px-5 py-4 text-slate-600">{{ $tpl->created_at->format('d/m/Y H:i') }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <button type="button" class="rounded-lg bg-indigo-500 px-3 py-2 text-xs font-semibold text-white hover:bg-indigo-600" data-open-edit data-id="{{ $tpl->id }}" data-nome="{{ $tpl->nome }}" data-ativo="{{ $tpl->ativo ? '1' : '0' }}">Editar</button>
                                <form method="POST" action="{{ route('adm.iaplataformas.destroy', $tpl) }}" onsubmit="return confirm('Deseja excluir esta plataforma?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        (function () {
            const modal = document.getElementById('iaplataformaModal');
            const openBtn = document.getElementById('openIaplataformaModal');
            const closeBtns = modal.querySelectorAll('[data-close-modal]');
            const form = document.getElementById('iaplataformaForm');
            const methodInput = document.getElementById('iaplataformaFormMethod');
            const title = document.getElementById('iaplataformaModalTitle');
            const nome = document.getElementById('iaplataformaNome');
            const ativo = document.getElementById('iaplataformaAtivo');
            const openModal = () => { modal.classList.remove('hidden'); modal.classList.add('flex'); };
            const closeModal = () => { modal.classList.add('hidden'); modal.classList.remove('flex'); };
            const resetForm = () => {
                form.action = "{{ route('adm.iaplataformas.store') }}";
            ...
            document.querySelectorAll('[data-open-edit]').forEach(button => {
                button.addEventListener('click', () => {
                    ...
                    resetForm();
                    form.action = `{{ url('/adm/ia-plataformas') }}/${id}`;
