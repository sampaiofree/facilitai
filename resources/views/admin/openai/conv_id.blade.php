        ...
        <div class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p class="text-xs text-slate-500">Contexto do vínculo usado para consultar a conversa.</p>
                </div>
                @if(($assistantLeadMatchesCount ?? 0) > 1)
                    <span class="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[11px] font-semibold text-amber-700">
                        {{ $assistantLeadMatchesCount }} vínculos encontrados; exibindo o mais recente
                    </span>
                @endif
            </div>
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-lg border border-slate-200 p-3">
                    ...
                    <p class="text-xs text-slate-500">ID {{ $assistant?->id ?? '-' }} | versão {{ $assistantLead->version ?? '-' }}</p>
                </div>
            </div>
