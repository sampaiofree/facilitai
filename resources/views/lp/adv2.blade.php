...
<head>
    ...
    <!-- ESTILO PARA X-CLOAK: FUNDAMENTAL PARA EVITAR FOUC ANTES DO ALPINE.JS INICIAR -->
    <style>
        [x-cloak] { display: none !important; }
    </style>
</head>

<!-- O BODY ENVOLVE TUDO E SÓ SERÁ VISÍVEL QUANDO O ALPINE INICIAR -->
<body class="antialiased text-gray-800 bg-slate-50 selection:bg-emerald-300 selection:text-emerald-900"
      x-cloak x-data="{ loaded: false }" x-init="$nextTick(() => loaded = true)">

    <div x-show="loaded" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">    
        <!-- CORPO PRINCIPAL -->
        <main class="mx-auto max-w-screen-2xl">
            ...
            <section id="hero" class="relative overflow-hidden bg-gradient-to-br from-indigo-900 via-indigo-800 to-indigo-700 text-white py-20">
                    <div class="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10">
                        ...
                        <!-- Conteúdo textual -->
                        <div class="w-full lg:w-1/2 space-y-6" x-data="{ enviado: false }">
                            ...
                            <form @submit.prevent="enviado = true" class="bg-white/10 backdrop-blur-md rounded-2xl p-6 space-y-4 max-w-md">
                                <div>
                                    <label for="nome" class="block text-sm font-medium text-indigo-100">Seu nome</label>
                                    <input id="nome" name="nome" type="text" required
                                        class="mt-1 block w-full rounded-lg border-none focus:ring-2 focus:ring-emerald-400 text-gray-900 p-3 placeholder-gray-400"
                                        placeholder="Digite seu nome completo">
                                </div>

                                <div>
                                    <label for="whatsapp" class="block text-sm font-medium text-indigo-100">WhatsApp</label>
                                ...
                                <button type="submit"
                                    class="w-full bg-emerald-500 hover:bg-emerald-600 transition font-semibold py-3 rounded-lg shadow-lg">
                                    Quero ver como funciona
                                </button>

                                <template x-if="enviado">
                                    <p class="text-emerald-300 text-sm mt-2 animate-fadeIn">
                                        Obrigado! Em breve entraremos em contato pelo WhatsApp 📲
                                    </p>
                                </template>
                            </form>
                        </div>
